...
for j in vehicle_list:
    capacity_list.append(data["vehicles"][j]["capacity"])
...
def nearest_neighbor(graph):
    ...
    visited_path, unvisited_check = visited(unvisited_neigh, current_neigh, tour, capacity)
    tours.append(visited_path)

    while unvisited_check:
        capacity_list.append(capacity_list[0])
        capacity_list.pop(0)
        capacity = capacity_list[0]
        current_neigh = 0
        tour = [current_neigh]
        visited_path, unvisited_check = visited(unvisited_check, current_neigh, tour, capacity)
        tours.append(visited_path)

    return tours

def visited(unvisited_neigh, current_neigh, tour, capacity):
    cost = 0
...
result = {}
paths = []
for k in range(len(result_tour)//len(vehicle_list)):
    paths.append("path" + str(k+1))

new_result = []
for x in range(len(vehicle_list)):
   new_result.append([])
print(result_tour[0])
print(new_result[0])
count = 0
while result_tour:
    for i in range(len(vehicle_list)):
        new_result[i].append(result_tour[count])
        result_tour.pop(0)
print(new_result)
'''
...
with open('level1b_output.json', 'w') as f_write:
